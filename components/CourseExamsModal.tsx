...
// Función para obtener los exámenes de un curso específico
const fetchCourseExams = async (courseId: string): Promise<Exam[]> => {
  try {
    const response = await courseClient.get(`/exams/course/${courseId}`);
    console.log("API exams response:", JSON.stringify(response.data, null, 2));
    ...
    <Portal>
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          ) : !exams || exams.length === 0 || !exams.some(exam => exam.published) ? (
            <View style={styles.emptyContainer}>
              {exams && exams.length > 0 ? (
                <Text style={styles.emptyText}>
                  Este curso tiene {exams.length} {exams.length === 1 ? 'examen' : 'exámenes'}, pero ninguno está publicado todavía.
                </Text>
              ) : (
