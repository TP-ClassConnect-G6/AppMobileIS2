...
// Tipo para la respuesta de la entrega de tarea
...
const TaskSubmissionModal = ({ visible, onDismiss, taskId, taskTitle, dueDate }: TaskSubmissionModalProps) => {
  ...
  // Función para enviar la entrega de la tarea
  const submitTask = async () => {
    ...
    try {
      ...
      const response = await courseClient.post(`/tasks/${taskId}/submissions`, formData, {
        headers: {
      ...
      const submissionResponse: TaskSubmissionResponse = response.data;
      ...
      let errorMessage = "No se pudo enviar la tarea. Intenta nuevamente.";
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400:
            ...
            break;
          case 403:
            errorMessage = "No tienes permisos para enviar esta tarea";
            break;
          case 404:
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent}>
          {submissionSuccess && submissionData ? (
            // Vista de éxito después de enviar la tarea
            <>
              <View style={styles.successHeader}>
                <Title style={styles.title}>¡Tarea Enviada!</Title>
                <Chip 
            ...
            <>
              <Title style={styles.title}>Enviar Tarea</Title>
              <Text style={styles.taskTitle}>{taskTitle}</Text>
              {dueDate && (
                <Text style={styles.dueDateText}>
                  Fecha límite: {formatSubmissionDate(dueDate)}
                  {isSubmissionLate() && " (Vencida)"}
                </Text>
              )}
              <Divider style={styles.divider} />
              ...
              {isSubmissionLate() && (
                <View style={styles.warningContainer}>
                  <Text style={styles.warningText}>
                    ⚠️ La fecha límite de entrega ya ha pasado. Esta será marcada como entrega tardía.
                  </Text>
                </View>
              )}
              ...
              {uploading && (
                <View style={styles.uploadingContainer}>
                  ...
                  <Text style={styles.uploadingText}>Enviando tarea...</Text>
                </View>
              )}
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  // Estilos para la vista de éxito
  successHeader: {
    flexDirection: 'row',
